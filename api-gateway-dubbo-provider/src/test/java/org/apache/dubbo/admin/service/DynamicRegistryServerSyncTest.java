...
public class DynamicRegistryServerSyncTest {

    @Mock
    private Registry registry;

    @InjectMocks
    private DynamicRegistryServerSync dynamicRegistryServerSync;

    private InterfaceRegistryCache interfaceRegistryCache = new InterfaceRegistryCache();

    @Before
    public void setUp() throws Exception {
        ReflectionTestUtils.setField(dynamicRegistryServerSync, "interfaceRegistryCache", interfaceRegistryCache);
    }

    @Test
    public void testAfterPropertiesSet() throws Exception {
        dynamicRegistryServerSync.startSubscribe("zookeeper://127.0.0.1:2181");
        verify(registry).subscribe(any(URL.class), any(DynamicRegistryServerSync.class));
    }

    @Test
    public void testDestroy() throws Exception {
        dynamicRegistryServerSync.destroy();
        verify(registry).unsubscribe(any(URL.class), any(DynamicRegistryServerSync.class));
    }

    @Test
    public void testNotify() {
        dynamicRegistryServerSync.notify(null);
        dynamicRegistryServerSync.notify(Collections.emptyList());
        ...
        dynamicRegistryServerSync.notify(Arrays.asList(consumerUrl, consumerUrl, providerUrl));
        ...
        when(consumerUrl.getUrlParam()).thenReturn(URLParam.parse(consumerUrlParam));
        dynamicRegistryServerSync.notify(Collections.singletonList(consumerUrl));
        ...
        dynamicRegistryServerSync.notify(Collections.singletonList(providerUrl));
